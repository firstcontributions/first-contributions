...
Muitos dos repositórios matrizes do GitHub (ou seja, aqueles que não são _Fork_ de nenhum outro repositório) tem um pequeno grupo de pessoas que podem fazer commits de modificações diretamente. Todos os outros contribuidores devem fazer um _for_ do repositório e fazer alterações no _fork_, em seguida, devem criar um Pull Request para solicitar um _Merge_ (mesclagem) das modificações feitas no repositório original.
