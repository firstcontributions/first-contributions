...
Bu proje, yeni başlayanların ilk katkılarını yapma sürecini basitleştirmeyi ve onlara yol göstermeyi amaçlamaktadır. Eğer ilk katkınızı yapmak istiyorsanız, aşağıdaki adımları takip edin.
...
Bu sayfanın üst kısmındaki "fork" butonuna tıklayarak bu depoyu çatallayın.
Bu işlem, bu deponun bir kopyasını kendi hesabınızda oluşturacaktır.
...
```bash
git clone git@github.com:kullanici-adiniz/first-contributions.git

```

burada `kullanici-adiniz` sizin GitHub kullanıcı adınızdır. Bu komutla, GitHub üzerindeki first-contributions deposunun içeriğini bilgisayarınıza kopyalamış oluyorsunuz.

## Bir dal (branch) oluşturun

Bilgisayarınızdaki proje dizinine gidin (eğer henüz orada değilseniz):
...
Şimdi `git switch` komutunu kullanarak yeni bir dal oluşturun:

```bash
git switch -c yeni-dal-isminiz
...
<details>
<summary> <strong>Eğer git switch kullanırken hata alıyorsanız, buraya tıklayın:</strong> </summary>

Eğer "Git: `switch` is not a git command. See `git –help`" hata mesajı alıyorsanız, muhtemelen git'in eski bir sürümünü kullanıyorsunuzdur.

Bu durumda, bunun yerine `git checkout` kullanmayı deneyin:

```bash
git checkout -b yeni-dal-isminiz

```

</details>

## Gerekli değişiklikleri yapın ve bu değişiklikleri kaydedin (commit)
...
`git add` komutunu kullanarak, oluşturduğunuz daldaki bu değişiklikleri ekleyin:
...
Şimdi `git commit` komutunu kullanarak bu değişiklikleri kaydedin:
...
`isminiz` yerine kendi adınızı yazın.

## Değişiklikleri GitHub'a gönderin (push)

`git push` komutunu kullanarak değişikliklerinizi gönderin:

```bash
git push -u origin dal-isminiz

```

`dal-isminiz` yerine daha önce oluşturduğunuz dalın ismini yazın.

<details>
<summary> <strong>Gönderirken (push) hata alıyorsanız, buraya tıklayın:</strong> </summary>
...
fatal: Authentication failed for '[https://github.com/&lt;your-username&gt;/first-contributions.git/](https://github.com/<your-username>/first-contributions.git/)'</pre>
Hesabınıza bir SSH anahtarı oluşturmak ve yapılandırmak için [GitHub'ın eğitimine](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account) gidin.
Ayrıca, uzak adresinizi kontrol etmek için 'git remote -v' komutunu çalıştırmak isteyebilirsiniz.
Eğer şuna benzer görünüyorsa:
<pre>origin	[https://github.com/kullanici-adiniz/your_repo.git](https://www.google.com/search?q=https://github.com/kullanici-adiniz/your_repo.git) (fetch)
origin	[https://github.com/kullanici-adiniz/your_repo.git](https://www.google.com/search?q=https://github.com/kullanici-adiniz/your_repo.git) (push)</pre>
Şu komutla değiştirin:
```bash
git remote set-url origin git@github.com:kullanici-adiniz/your_repo.git

```


Aksi takdirde hala kullanıcı adı ve şifre sorulmaya devam edilecek ve kimlik doğrulama hatası alacaksınız.
...
Tebrikler! Bir katkıda bulunurken sıkça karşılaşacağınız standart *fork -> clone -> edit -> pull request* iş akışını tamamladınız!

Katkınızı kutlayın ve [web uygulamasına](https://firstcontributions.github.io/#social-share) giderek arkadaşlarınızla ve takipçilerinizle paylaşın.

Daha fazla pratik yapmak isterseniz, [kod katkılarına (code contributions)](https://github.com/roshanjossey/code-contributions) göz atın.

Şimdi sizi diğer projelere katkıda bulunmaya başlatalım. Başlamanız için kolay sorunlara (issue) sahip projelerin bir listesini derledik. [Web uygulamasındaki proje listesine](https://firstcontributions.github.io/#project-list) göz atın.

### [Ek materyaller](https://www.google.com/search?q=docs/additional-material/git_workflow_scenarios/additional-material.md)

## Diğer Araçları Kullanan Eğitimler
