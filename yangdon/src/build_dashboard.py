"""대시보드 데이터 생성 → yangdon/dashboard/index.html.

통합 파이프라인(CCTV 발정관찰 + 관리요인 → 무발정 위험/처방)과 시간 윈도우
발정 시작점 탐지 결과를 하나의 JSON 으로 만들어 템플릿에 주입한다.

    python yangdon/src/build_dashboard.py                       # 합성 시연
    python yangdon/src/build_dashboard.py <cctv_dir> <mgmt.csv> # 실데이터
"""
...
OUT_HTML = os.path.join(ROOT, "yangdon", "dashboard", "index.html")
TEMPLATE = os.path.join(ROOT, "yangdon", "dashboard", "template.html")
