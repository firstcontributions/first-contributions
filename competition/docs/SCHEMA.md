...
competition/src/parse_aihub.py --selftest` (스키마대로 만든 합성 샘플로 라운드트립).
...
**모델 데모**: `python competition/src/model_71763.py [라벨디렉터리]`
...
**실행**: `python competition/src/model_71471_estrus.py [라벨디렉터리]`
...
실행: `python competition/src/model_gilt_anestrus.py [gilt.csv]`
...
- 실행: `python competition/src/pipeline_gilt.py [cctv_dir] [mgmt.csv]`
...
```bash
python competition/src/parse_aihub.py --selftest              # 스키마 검증
python competition/src/parse_aihub.py 71763 <라벨디렉터리>     # JSON → CSV
python competition/src/parse_aihub.py 622   <라벨디렉터리>     # XML  → CSV
```
