...
python competition/src/parse_edinburgh.py
...
```bash
python competition/src/build_dashboard.py                       # 합성 시연 데이터로 생성
python competition/src/build_dashboard.py <cctv_dir> <mgmt.csv> # 실데이터로 생성
```
...
4. `bash competition/run_pipeline.sh` 로 라벨 다운로드→추출→구조점검
   (JSON 스키마 확인 후 파서로 `data/train.csv` 생성 → `eda.py`/`train.py`)

> 환경 시크릿은 **세션 시작 시 주입**되므로, 등록 후에는 반드시 **새 세션**에서
> 진행해야 키가 적용된다.

스모크 테스트: `python competition/tests/smoke_test.py`
...
```bash
python competition/src/aihub.py search 양돈       # 데이터셋 검색 (API 키 불필요)
python competition/src/aihub.py tree 71763        # 파일 트리·filekey 확인 (키 불필요)
# 다운로드는 AIHUB_APIKEY 환경변수 + 데이터셋 활용신청 승인 필요
export AIHUB_APIKEY="발급받은_키"
python competition/src/aihub.py download 71763 528771,528774   # 라벨링데이터만
```
...
python competition/src/generate_data.py   # data/train.csv 생성
python competition/src/eda.py             # outputs/ 에 EDA 결과
python competition/src/train.py           # 회귀+분류 모두 (reg / clf 인자로 개별 실행)
```
...
2. `python competition/src/eda.py` → 분포·결측·상관 점검.
3. `python competition/src/train.py` → 베이스라인 성능 확인 후 개선.
