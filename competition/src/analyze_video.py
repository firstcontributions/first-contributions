...
# conf 0.25 + max_age 40 조합이 기본(0.35/8) 대비 과분할 59% 개선.
# (카메라 모션 보상은 같은 조건에서 14%에 그쳐 기본 비활성)
#
# ⚠️ 박스 병합은 기본 **비활성**이다. 스윕에서는 이득으로 보였으나(앞 168프레임),
# 군사 사육 전체 영상에 적용하니 붙어 있는 개체까지 연쇄 병합해 마릿수가 5→1 로
# 붕괴했다. 병합 규칙에 안전장치를 넣었지만(연쇄 금지·중앙값 크기 상한), 개체가
# 밀집한 영상에서는 위험이 남아 옵션으로 둔다.
CONF = 0.25
MAX_AGE = 40
IOU_THR = 0.25
MERGE_BOXES = False
