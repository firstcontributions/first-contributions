...
# 국내 179개 농장 월별 실측(2020~2023)으로 이 가정을 대조했다
# — src/farm_monthly.py. **여름 불임은 실재하지만 크기는 크지 않다.**
#
#   분만율(교배월 기준)  여름 7·8·9월 81.1%  vs  겨울 1·2·3월 84.1%   −2.97%p
#   7일내재귀율                    89.5%  vs            89.7%   −0.2%p
#   평균이유                       10.4두 vs            10.6두   −0.2두
#
# 주의 둘:
#  1) **분만율은 분만 시점에 기록된다.** 그대로 보면 11월이 최저라 여름 효과가
#     안 보인다. 임신 114일을 빼서 교배월로 되돌려야 7·8월이 드러난다.
#  2) 이 −2.97%p 는 **농장 중앙값의 계절 차이**이고, THI 구간별 인과 효과가
#     아니다. 위 WEI_PENALTY 는 여전히 문헌 기반 가정이며, 실측 온습도와
#     개체 WEI 가 짝지어진 자료가 있어야 보정할 수 있다.
SEASONAL_MEASURED = {
    "farrowing_rate_summer_gap_pp": -2.97,
    "return7d_summer_gap_pp": -0.2,
    "weaned_summer_gap": -0.25,
    "basis": "국내 179농장 월별 2020~2023 · 분만율은 교배월로 환산",
}
