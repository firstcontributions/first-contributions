...
ھاڻي شاخ ٺاھيو حڪم `git switch` استعمال ڪندي:

```bash
git switch -c <توهان جي-نئين-شاخ-جو-نالو>
```
...
<details>
<summary> <strong>جيڪڏهن توهان کي <code>git switch</code> استعمال ڪندي ڪا غلطي ملي ٿي، ته هتي ڪلڪ ڪريو:</strong> </summary>

جيڪڏهن غلطي جو پيغام "Git: `switch` is not a git command. See `git –help`" ظاهر ٿئي ٿو، اهو ممڪن آهي ڇو ته توهان git جو پراڻو ورزن استعمال ڪري رهيا آهيو.

ان صورت ۾، بدران <code>git checkout</code> استعمال ڪرڻ جي ڪوشش ڪريو:

```bash
git checkout -b <توهان جي-نئين-شاخ-جو-نالو>
```
