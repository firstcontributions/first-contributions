...
#### _Baca ini dalam [bahasa lain](docs/translations/Translations.md)._
...
Proyek ini bertujuan untuk menyederhanakan dan memandu langkah pemula dalam membuat kontribusi pertama mereka. Jika Anda ingin membuat kontribusi pertama Anda, ikuti langkah-langkah di bawah ini.

_Jika Anda tidak nyaman dengan antarmuka baris perintah (command line), [berikut adalah tutorial menggunakan alat GUI.](#tutorial-menggunakan-alat-lain)_

<img align="right" width="300" src="https://firstcontributions.github.io/assets/Readme/fork.png" alt="fork repositori" />

#### Jika Anda belum memiliki git di komputer Anda, [pasang git terlebih dahulu](https://docs.github.com/en/get-started/quickstart/set-up-git).

## Fork repositori ini

Fork repositori ini dengan mengklik tombol fork di bagian atas halaman ini.
Langkah ini akan membuat salinan repositori ini di akun Anda.

## Clone repositori

<img align="right" width="300" src="https://firstcontributions.github.io/assets/Readme/clone.png" alt="clone repositori" />

Sekarang clone repositori hasil fork ke komputer Anda. Buka akun GitHub Anda, buka repositori hasil fork, klik tombol code, lalu ke tab SSH/HTTPS dan klik ikon _copy url to clipboard_.

Buka terminal dan jalankan perintah git berikut:

```bash
git clone "url yang baru saja Anda salin"
```

di mana "url yang baru saja Anda salin" (tanpa tanda kutip) adalah URL ke repositori ini (hasil fork Anda dari proyek ini). Lihat langkah-langkah sebelumnya untuk mendapatkan URL tersebut.
...
```bash
git clone git@github.com:ini-anda/first-contributions.git
```

di mana `ini-anda` adalah nama pengguna (username) GitHub Anda. Di sini Anda menyalin isi repositori first-contributions dari GitHub ke komputer Anda.

## Buat branch baru

Pindah ke direktori repositori di komputer Anda (jika Anda belum berada di sana):

```bash
cd first-contributions
```

Sekarang buat branch baru menggunakan perintah `git switch`:

```bash
git switch -c nama-branch-baru-anda
```
...
<details>
<summary> <strong>Jika Anda mendapatkan error saat menggunakan git switch, klik di sini:</strong> </summary>

Jika pesan error "Git: `switch` is not a git command. See `git –help`" muncul, kemungkinan besar karena Anda menggunakan versi git yang lebih lama.

Dalam kasus ini, cobalah gunakan perintah `git checkout`:
...
## Lakukan perubahan yang diperlukan dan simpan (commit) perubahan tersebut

Sekarang buka file `Contributors.md` di penyunting teks (text editor), lalu tambahkan nama Anda ke dalamnya. Jangan tambahkan di awal atau akhir file. Letakkan di mana saja di antaranya. Sekarang, simpan file tersebut.

<img align="right" width="450" src="https://firstcontributions.github.io/assets/Readme/git-status.png" alt="git status" />

Jika Anda pergi ke direktori proyek dan menjalankan perintah `git status`, Anda akan melihat ada perubahan.

Tambahkan perubahan tersebut ke branch yang baru Anda buat menggunakan perintah `git add`:
...
Unggah (push) perubahan Anda menggunakan perintah `git push`:

```bash
git push -u origin nama-branch-anda
```

ganti `nama-branch-anda` dengan nama branch yang Anda buat sebelumnya.

<details>
<summary> <strong>Jika Anda mengalami masalah/error saat melakukan push, klik di sini:</strong> </summary>

- ### Error Otentikasi (Authentication Error)
     <pre>remote: Support for password authentication was removed on August 13, 2021. Please use a personal access token instead.
  remote: Please see https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/ for more information.
  fatal: Authentication failed for 'https://github.com/&lt;nama-pengguna-anda&gt;/first-contributions.git/'</pre>
  Buka [Tutorial GitHub](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account) untuk membuat dan mengonfigurasi SSH key pada akun Anda.

  Selain itu, Anda mungkin ingin menjalankan 'git remote -v' untuk memeriksa alamat remote Anda.
  
  Jika terlihat seperti ini:
  <pre>origin	https://github.com/nama-pengguna-anda/repo_anda.git (fetch)
  origin	https://github.com/nama-pengguna-anda/repo_anda.git (push)</pre>
  
  ubah dengan menggunakan perintah ini:
  ```bash
  git remote set-url origin git@github.com:nama-pengguna-anda/repo_anda.git
  ```
  Jika tidak, Anda akan tetap dimintai nama pengguna dan kata sandi serta mendapatkan error otentikasi.
</details>

## Ajukan perubahan Anda untuk ditinjau (Pull Request)

Jika Anda pergi ke repositori Anda di GitHub, Anda akan melihat tombol `Compare & pull request`. Klik tombol tersebut.

<img style="float: right;" src="https://firstcontributions.github.io/assets/Readme/compare-and-pull.png" alt="bandingkan dan buat pull request" />

Sekarang kirimkan pull request tersebut.
...
Segera saya akan menggabungkan (merge) semua perubahan Anda ke branch utama dari proyek ini. Anda akan mendapatkan email pemberitahuan setelah perubahan tersebut digabungkan.

## Ke mana harus melangkah setelah ini?

Selamat! Anda baru saja menyelesaikan alur kerja standar _fork -> clone -> edit -> pull request_ yang akan sering Anda temui sebagai kontributor!

Rayakan kontribusi Anda dan bagikan dengan teman serta pengikut Anda melalui [web app](https://firstcontributions.github.io/#social-share).

Jika Anda ingin latihan lebih banyak, lihat [kontribusi kode](https://github.com/roshanjossey/code-contributions).

Sekarang mari mulai berkontribusi pada proyek lain. Kami telah menyusun daftar proyek dengan issue mudah yang bisa Anda mulai. Lihat [daftar proyek di web app](https://firstcontributions.github.io/#project-list).

### [Materi Tambahan](docs/additional-material/git_workflow_scenarios/additional-material.md)
