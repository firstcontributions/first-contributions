...
当遇到合并冲突时，Git 会通过在文件中为问题区域添加开始标记和其他分支名称来标记冲突。

第一个标记后的内容来自你当前的工作分支。Git 会告诉我们更改来自哪个分支。一个分隔行将两个冲突的更改分开。
...
`HEAD`：表示合并冲突行的开始。第一组行来自你试图合并更改的文件。  
分隔线：表示用于比较的断点。分隔用户提交的更改（上方）和来自合并的更改（下方），以便直观地看到差异。  
`branch-name`：表示合并冲突行的结束。

你可以通过编辑文件来解决冲突，然后手动合并 Git 难以合并的部分。这可能意味着丢弃你的更改或别人的更改，或者两者的混合。你还需要删除文件中的冲突标记行。
