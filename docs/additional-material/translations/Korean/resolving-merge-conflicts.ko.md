...
병합 충돌이 발생하면 Git은 문제가 되는 부분을 시작 표시와 다른 브랜치 이름을 함께 사용해 표시합니다.

이때 여러분이 현재 작업중인 브랜치가 먼저 표기됩니다. 시작 표시 뒤를 보면 어느 브랜치에서 변경사항이 반영되었는지 알 수 있습니다.
구분선 기호는 충돌이 발생한 부분을 각각 구분해줍니다.
...
`HEAD`: 병합 충돌이 시작되는 곳을 표시합니다. 여러분이 병합하고자하는 변경한 라인들로 이루어진 부분이 첫번째로 표기됩니다.
구분선: 비교하기 위한 구분선을 나타냅니다. 쉽게 차이를 파악할 수 있도록 사용자가 커밋한 변경사항(위)과 병합을 위해 로드된 부분(아래)으로 구분되어 있습니다.
`branch-name`: 병합 충돌이 발생한 마지막 위치를 표시합니다.
...
그리고 해당 파일에서 Git 충돌 마커 줄을 제거해야 합니다.
