...
REQ="$ROOT/yangdon/requirements.txt"
...
if [ -n "${AIHUB_APIKEY:-}" ]; then
  ...
  echo "[session-start] 다음: python $ROOT/yangdon/run_pipeline.sh 로 다운로드→EDA→학습"
else
  echo "[session-start] AIHUB_APIKEY 없음 → 환경 시크릿 등록 후 새 세션에서 진행"
  echo "[session-start] (키 없이도 python yangdon/src/aihub.py search 양돈 등 조회는 가능)"
fi
